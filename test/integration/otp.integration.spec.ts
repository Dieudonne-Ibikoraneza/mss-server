...
/**
 * The OTP limiter against real Redis, where the interesting failures live: a
 ...
 */
describe('one-time codes under concurrent guessing', () => {
  const url = process.env.REDIS_URL;
  if (!url) throw new Error('REDIS_URL is not set — these tests need a Redis to talk to.');

  const redisClient = new Redis(url, { maxRetriesPerRequest: 3 });
