...
export class GeminiRoomTileProvider implements RoomTileEditProvider {
  ...
  async generate(input: RoomTileEditInput): Promise<GeneratedImage | null> {
    ...
    return callGeminiImageModel(this.logger, this.apiKey, this.model, [
      {
        text: `Edit the FIRST attached photo, a real photo of the customer's own room. Replace ONLY the floor surface with the tile shown in the SECOND attached photo — ${input.product.name} (${input.product.collection}, ${input.product.size})${description}. This tile's real, exact physical size is ${input.product.size} — a known fact, not a guess. Use this exact size, scaled against the real room's own furniture, doorways, and walls in the first photo, to decide how large each tile and its grout lines should appear on the floor; never infer the tile's size from the second photo's crop or aspect ratio, since that photo shows only its color, pattern, and finish. Be attentive and deliberate about getting this proportion right rather than eyeballing it. Keep the exact same room in the result: the same walls, furniture, windows, ceiling, lighting, camera angle, and perspective as the first photo — this is a photo edit, not a newly generated scene. Reproduce the tile's true color, pattern, and finish exactly as shown in the second photo, at the real size stated above. Do not add text, labels, watermarks, swatches, or a collage; return one edited photo of the room only.`,
      },
